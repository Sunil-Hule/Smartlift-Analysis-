...
# List all CSV files in the MetaMotion directory
files = glob("../../data/raw/MetaMotion/*.csv")

# Define data path for consistent access
data_path = "../../data/raw/MetaMotion/"

# Extract metadata from the first file in the list for demonstration purposes
f = files[0]
participant = f.split("-")[0].replace(data_path, "")
label = f.split("-")[1]
category = f.split("-")[2].rstrip("123").rstrip("_MetaWear_2019")

# Read the first file and add metadata as new columns
df = pd.read_csv(f)
...
# Define a function to encapsulate the above logic for reading and categorizing data
...
# Use the defined function to process all files
...
# Resample data based on frequency specifications
sampling = {
    "acc_x": "mean",
    "acc_y": "mean",
    "acc_z": "mean",
...
data_resampled = pd.concat(
    [
        ...
        for df in data_merged.groupby(pd.Grouper(freq="D"))
    ]
)
