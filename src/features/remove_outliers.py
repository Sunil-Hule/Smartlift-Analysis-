...
# Load the processed data from the specified pickle file
df = pd.read_pickle("../../data/interim/01_data_processed.pkl")

# Get the first 6 columns of the dataframe which are considered for detecting outliers
...
# Set the plotting style to 'fivethirtyeight' for aesthetics
plt.style.use("fivethirtyeight")

# Adjust plotting parameters for better visualization
plt.rcParams["figure.figsize"] = (20, 5)  # Set the size of the figure
plt.rcParams["figure.dpi"] = 100  # Set the dots-per-inch (dpi) value
plt.rcParams["lines.linewidth"] = 2  # Set the width of the lines

# Boxplot for 'acc_x' column grouped by 'label' column
df[["acc_x", "label"]].boxplot(by="label", figsize=(20, 10))

# Boxplot for 'gyr_y' column grouped by 'label' column
df[["gyr_y", "label"]].boxplot(by="label", figsize=(20, 10))

# Boxplot for the first three columns in the outlier_columns list grouped by 'label'
df[outlier_columns[:3] + ["label"]].boxplot(by="label", figsize=(20, 10), layout=(1, 3))

# Boxplot for the next three columns in the outlier_columns list grouped by 'label'
df[outlier_columns[3:] + ["label"]].boxplot(by="label", figsize=(20, 10), layout=(1, 3))
